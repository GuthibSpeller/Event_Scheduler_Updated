<?php
    class Controller {
        ...
        public function edit() {
            if(isset($_POST['id'])) {
                $id = (int) $_POST['id'];
                // redirect to hyperlink.php with id so modal opens
                header("Location: ../frontEnd/hyperlink.php?id=$id");
                exit();
            }
        }
        ...
        public function update() {
            ...
            if (strtotime($event_end) < strtotime($event_start)) {
                $location = "../frontEnd/hyperlink.php?error=invalid_date&id=$id"; // id if updating
                header("Location: $location");
                exit();
            }
        ...
        public function sortAndfilter ($events, $sortType, $filterType) {
            ...
            if ($filterType == "pending" && $sortType == "nearest") {
                $sql = "SELECT * FROM eventlist WHERE event_status = 'pending' AND event_author = 'Hyperlink'";
                $stmt = $this->connection->prepare($sql);
                $stmt->execute();
                $result = $stmt->get_result();
                $events = $result->fetch_all(MYSQLI_ASSOC);
            ...
            if ($filterType == "completed" && $sortType == "nearest") {
                $sql = "SELECT * FROM eventlist WHERE event_status = 'completed' AND event_author = 'Hyperlink'";
                $stmt = $this->connection->prepare($sql);
                $stmt->execute();
                $result = $stmt->get_result();
                $events = $result->fetch_all(MYSQLI_ASSOC);
            ...
            if ($filterType == "cancelled" && $sortType == "nearest") {
                $sql = "SELECT * FROM eventlist WHERE event_status = 'cancelled' AND event_author = 'Hyperlink'";
                $stmt = $this->connection->prepare($sql);
                $stmt->execute();
                $result = $stmt->get_result();
                $events = $result->fetch_all(MYSQLI_ASSOC);
