<?php
    class Controller {
        ...
        public function delete() {
            if (isset($_GET['id'])) {
                $id = intval($_GET['id']);
                $stmt = $this->connection->prepare("DELETE FROM eventlist WHERE id=? AND event_author='BYTe'");
                if($stmt) {
        ...
        public function update() {
            ...
            if($id) {
                $sql = "UPDATE eventlist SET event_title=?, 
                                            ...
                                            event_status=?, 
                                            event_author=? 
                                            WHERE id=? AND event_author='BYTe'";
                $stmt = $this->connection->prepare($sql);
                if ($stmt) {
                    $stmt->bind_param(
                        "sssssssi",
        ...
        public function update_take_data ($id) {
            $query = "SELECT * FROM eventlist WHERE id=? AND event_author='BYTe'";
